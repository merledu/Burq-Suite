...
class Gpio[A <: AbstrRequest, B <: AbstrResponse]
  ...
  gpioRegTop.io.req <> io.req
  io.rsp <> gpioRegTop.io.rsp

  reg2hw <> gpioRegTop.io.reg2hw
  hw2reg <> gpioRegTop.io.hw2reg
...
object GpioDriver extends App {
  implicit val config = WishboneConfig(32,32)
  println(ChiselStage.emitVerilog(new Gpio(new WBRequest(), new WBResponse())))
}