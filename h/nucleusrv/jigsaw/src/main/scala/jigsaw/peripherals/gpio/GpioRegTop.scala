...
class GpioRegTop[A <: AbstrRequest, B <: AbstrResponse]
  ...
  // Assuming gpio is always ready to accept request
  io.req.ready := true.B

  val AW = 6    // assuming we will not have more than 2^6 = 64 csr registers in gpio
  val DW = 32
  val DBW = DW/8

  val reg_we, reg_re, reg_error, reg_ack = Wire(Bool())
  ...
  // req.fire() -> when req.valid (input) == true and req.ready (output) == true
  reg_we := Mux(io.req.fire(), io.req.bits.isWrite, false.B)
  reg_re := Mux(io.req.fire(), !io.req.bits.isWrite, false.B)
  reg_wdata := io.req.bits.dataRequest
  ...
  io.rsp.bits.ackWrite := reg_ack

  reg_rdata := reg_rdata_next
  reg_error := addr_miss || wr_err
  reg_ack := io.req.bits.isWrite
  ...
  val addr_hit = Wire(Vec(GpioRegistersMap.all.size, Bool()))
  for (i <- GpioRegistersMap.all.indices) {
    addr_hit(i) := reg_addr === GpioRegistersMap.all(i).asUInt()
  }

  addr_miss := Mux(reg_re || reg_we, ~addr_hit.contains(true.B), false.B)

  val GPIO_PERMIT = Wire(Vec(14, UInt((DW/8).W)))
  // All GPIO registers support only word writes
  for(i <- 0 until 14) {
